...
| `CompatConfig` | Contents of `[compat]`. `houdini: Option<String>` is a Cargo-style range; `houdini_min()` extracts its lower bound for Python ABI selection. `platforms: Vec<String>` declares the native platforms this package supports — must reference platforms used in `[stage.platform.*]` entries. |
...
| `ScriptEntry`, `ScriptEnv` | Untagged enum: `Plain(String)` for the shorthand form, `WithEnv { cmd: EnvValueSpec, python?, requirements? }` for the table form. `resolve_cmd(host_os)` picks the matching variant when `cmd` is conditional. Accessors `python()`, `requirements()`, `needs_venv()` work on both arms. |
