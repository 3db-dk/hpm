...
| `Registry` (async trait) | Registry abstraction. `ApiRegistry` and `GitRegistry` implement it. Note `get_versions` returns one entry per **build**, not per version — a version released for two platforms is two entries — because `get_version` needs the full set to pick the host's via `select_build_for_host`. Callers wanting one row per version go through `RegistrySet`, not this trait. |
| `RegistrySet` | Composite that fans requests out to every configured registry. `*_in` variants (`resolve_in`, `get_version_in`, `get_versions_in`) restrict the lookup to one named registry, honouring a dependency's `registry` pin; an unconfigured name is `UnknownRegistry` rather than a fallback to the rest of the set. `get_versions` and `search` collapse each `(name, version)`'s per-platform builds to one entry, preferring the host's build then a universal one — so these are *version* views, unlike the `Registry` trait method underneath. A version with no build for this host still lists (ruling it uninstallable is `select_build_for_host`'s job at install time), and a live build always wins over a yanked one at the same version so `highest_matching` still sees it. `search` collapses per registry, so a package served by two registries still shows both. |
| `global`, `global::ledger` | `hpm global` installs into Houdini's user preferences. `build_houdini_package` is shared with the project emitter; `Ledger` records what was installed per Houdini version and doubles as a `hpm clean` GC root. |
| `houdini_prefs` | Per-platform Houdini user-preferences directory resolution, including `HOUDINI_USER_PREF_DIR` and its `__HVER__` placeholder. The only hpm module that knows a Houdini *installation's* conventions. |
| `ArchiveFetcher` | Downloads and extracts registry-hosted archives. |
| `exec_mode` | The executable-bit rule, in one place for its two callers. `repaired_mode(mode, path)` / `looks_executable(path)` answer "is this a program that lost its bit?" per entry as an archive extracts; `ensure_repaired(package_dir)` applies the same rule across a tree that is **already** installed — the case extraction can never reach, since a package at a version you already have is never extracted again. `ensure_repaired` sweeps once per tree, stamping `<name>@<version>.exec-modes` beside (never inside) the package directory, because a package's checksum is computed over its contents. Embedders that spawn programs out of an installed tree without going through a sync should call it — or `StorageManager::repair_exec_modes()` to sweep the whole store at once, which is what an embedder wants at startup so a user's installs are repaired by *its* update rather than by their stumbling onto the right screen. `mirror_read_bits(mode)` exposes just the widening (`0o644` -> `0o755`) for an embedder about to spawn a path its manifest declares as a program — proof no content sniff can produce, covering what a sweep structurally cannot reach (a dev copy it doesn't walk, a file that appeared after the tree was stamped). `forget_repair` drops the stamp when a tree is removed. |
...
| `NativePackageTarget`, `normalize_hpackage_version`, `validate_hpackage_descriptor` | Which consumer the bundled `{slug}.json` is generated for. `generate_houdini_native_package_for(SideFxHpackage)` (behind `hpm pack --sidefx`) emits the form SideFX's hpackage repository accepts — the trimmed version and `major.minor` `enable` operands — and errors instead of emitting a bound whose meaning the rewrite would change. `Generic` is what `generate_houdini_native_package` emits. `validate_hpackage_descriptor` checks a descriptor `hpm pack --sidefx` is about to ship against the same rules from the other end — needed because a hand-written `{slug}.json` is shipped verbatim and never passes through the generator. It reads named fields out of the JSON rather than deserializing into `HoudiniNativePackage`, since a hand-written descriptor exists to express what hpm does not model, and returns the `hpackage.version` SideFX will store. |
