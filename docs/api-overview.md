...
| `ScriptEntry`, `ScriptEnv` | Untagged enum: `Plain(String)` for the shorthand form, `WithEnv { cmd: EnvValue, python?, requirements?, package_env }` for the table form. `resolve_cmd(host_os)` picks the matching variant when `cmd` is conditional. Accessors `python()`, `requirements()`, `needs_venv()`, `uses_package_env()` work on both arms. `package_env` (`package-env` in TOML) opts the script into the project's full resolved environment — see `ProjectManager::resolve_package_env`. |
