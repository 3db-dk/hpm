...
# Update hpm.toml to reflect the minimal structure
cat > hpm.toml << 'EOF'
[package]
name = "config-package"
version = "0.1.0"
...
hpm init dependency-demo --description "Dependency management examples"
cd dependency-demo
```
...
hpm add material-library --version "^1.5.0"  # Compatible with 1.5.x
...
hpm add custom-nodes --git "https://github.com/studio/custom-nodes" --tag "v1.0.0"
```
...
hpm update numpy --version ">=1.25.0,<2.0.0"
```
...
- [ ] Package validates successfully (`hpm check`)
...
hyperfine 'hpm install' --warmup 3
```
