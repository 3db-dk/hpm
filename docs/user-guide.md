...
- Because it runs through a shell, `cmd` is shell syntax, and the two hosts do not share one: `sh` expands `$VAR`, while `cmd.exe` means nothing by it and spells the same thing `%VAR%`. Give each host the spelling its shell reads with a conditional `cmd` keyed on `when.os` rather than expecting one string to work on both.
- Forwarded `ARGS` are quoted per argument for that shell, so spaces, quotes and backslashes reach the script as written. One Windows case has no escape and is not attempted: `cmd.exe` expands `%NAME%` even inside double quotes, and its command-line parser — unlike a batch file's — offers no way to suppress it, so an argument containing `%NAME%` where `NAME` is set in the environment arrives expanded. An unmatched `%`, or a name that is not set, passes through as written.
